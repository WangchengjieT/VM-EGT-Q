function [lev, state_all, Q_table_all] = simulation_Q_new_cost(vicsek_para, qlearning_para, mc_para, if_plot)
%     almost the same as simulation_Q, with new cost
%      
    ...
    state_all(:, 6) = atan2(imag(veloc_temp), real(veloc_temp));
    % 7
    ...
    % 9
    state_all(:, 9) = state_all(:, 4) .* state_all(:, 5) ./ (L ^ 2);
    % 10
    ...
    while flag_temp
        ...
        % 3
        state_all(:, 3, t) = state_all(:, 6, t - 1) .* state_all(:, 4, t - 1) + state_all(:, 7, t - 1);
        ...
        veloc_temp = sum((adj_mat_temp + eye(N)) .* veloc_temp.', 2);
        state_all(:, 6, t) = atan2(imag(veloc_temp), real(veloc_temp));
        % 7
        ...
        % 9
        state_all(:, 9, t) = state_all(:, 4, t) .* state_all(:, 5, t) ./ (L ^ 2);
        % 10
        ...
        if ~mod(t, 200)
            fprintf(['第%d组参数, alpha = %3.1f, r = %5.2f, eta = %3.1f, 实验: %d/%d, 进度: %3.1f, ' ...
                '合作率: %5.2f, 同步率: %5.2f \n'], group_ind, al, r, eta, b_mc, B, t, level(t, 1), level(t, 2));
        end
    ...
    if (lev(2) * (1 - lev(2)))
        ...
        lev(2:3) = mean(level(burn_in + 1 + (1:mc_step), :), 1);
    else
        for t = lev(1) + 1 + (1:more_step)      
    
            state_all(:, 1, t) = mod(state_all(:, 1, t - 1) + v0 .* cos(state_all(:, 3, t - 1)), L);
            state_all(:, 2, t) = mod(state_all(:, 2, t - 1) + v0 .* sin(state_all(:, 3, t - 1)), L);
            if lev(2)
                state_all(:, 3, t) = state_all(:, 6, t - 1) + state_all(:, 7, t - 1);
            else
                state_all(:, 3, t) = state_all(:, 7, t - 1);
            end
            adj_mat_temp = is_neighbour(state_all(:, 1:2, t), r, 0);
            state_all(:, 5, t) = sum(adj_mat_temp, 2);
            veloc_temp = exp(state_all(:, 3, t) .* 1i);
            ...
            state_all(:, 6, t) = atan2(imag(veloc_temp), real(veloc_temp));
            if lev(2)
                state_all(:, 7, t) = unifrnd(-eta, eta, N, 1);
            else
                state_all(:, 7, t) = unifrnd(0, 2 * pi, N, 1);
            end
    
            if ~mod(t, 200)
                fprintf(['第%d组参数, alpha = %3.1f, r = %5.2f, eta = %3.1f, 实验: %d/%d, 进度: %3.1f, ' ...
                    '合作率: %5.2f, 同步率: %5.2f \n'], group_ind, al, r, eta, b_mc, B, t, lev(2), level(t, 2));
            end      
        end
        
        lev(3) = mean(level(lev(1) + 1 + (1:more_step), 2));
    end
